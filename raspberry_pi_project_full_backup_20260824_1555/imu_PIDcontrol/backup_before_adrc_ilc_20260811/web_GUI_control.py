...
function updateControllerMode(){
  ...
  Array.from(target.options).forEach(option=>{option.disabled=demo && option.value!=='sine';});
  if(demo && target.value!=='sine'){
    target.value='sine';
    updateJointLabels();
  }
}

...
<div class="container">
  <div>
    ...
    <div class="card">
      ...
      <p class="small">可選擇肘或肩關節並觀察三路 Desired PWM；只有按鍵確認且 ESP32 ARM 成功後，實際 Output 才會啟用。</p>
    </div>
    ...
    <div class="card">
      ...
      <div class="grid2">
        ...
        <div><label>Assist delay 秒</label><input id="assistDelayInput" value="0.10"></div>
        ...
        <div><label>展示模式 PWM 上限</label><input id="demoOutputLimitInput" value="15" min="0" max="30"></div>
        <div><label>ADRC 控制頻寬 ωc (rad/s)</label><input id="adrcWcInput" value="2.0"></div>
        <div><label>ADRC 觀測器頻寬 ωo (rad/s)</label><input id="adrcWoInput" value="8.0"></div>
        <div><label>ADRC 輸入增益 b0</label><input id="adrcB0Input" value="1.0"></div>
        <div><label>ILC 學習增益 L</label><input id="ilcGainInput" value="0.08"></div>
        <div><label>ILC 忘卻因子 λ</label><input id="ilcForgettingInput" value="0.98"></div>
        <div><label>ILC Q-filter 視窗</label><input id="ilcQWindowInput" value="9"></div>
        <div><label>ILC 單周期更新上限</label><input id="ilcUpdateLimitInput" value="3"></div>
        <div><label>ILC learned PWM 上限</label><input id="ilcLearnedLimitInput" value="20"></div>
      </div>
  ...
  <div>
    <div class="card">
      ...
      <div class="metrics">
        ...
        <div class="metric"><div class="metric-title">前饋輸出</div><div class="metric-value" id="feedforwardOutput">--</div></div>
        ...
        <div class="metric"><div class="metric-title">ADRC 擾動估測</div><div class="metric-value" id="adrcDisturbance">--</div></div>
        <div class="metric"><div class="metric-title">ADRC 速度估測</div><div class="metric-value" id="adrcVelocity">--</div></div>
        <div class="metric"><div class="metric-title">ADRC output</div><div class="metric-value" id="adrcOutput">--</div></div>
        <div class="metric"><div class="metric-title">ILC learned output</div><div class="metric-value" id="ilcOutput">--</div></div>
        <div class="metric"><div class="metric-title">ILC cycle RMSE</div><div class="metric-value" id="ilcRmse">--</div></div>
...
async function requestJSON(url,options={}){
  ...
  try{
    ...
    return {ok:false,message:e.name==='AbortError'?'伺服器回應逾時，請檢查樹莓派連線。':'無法連線到 Flask Server：'+e.message};
  }finally{clearTimeout(timeout);}
}
async function postJSON(url,data){return await requestJSON(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data||{})});}
async function postJSONLong(url,data){return await requestJSON(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data||{}),timeoutMs:30000});}
...
function updatePage(data){
  ...
  if(data.frame){
    ...
    setText('feedforwardOutput',(f.feedforward_output ?? 0).toFixed(1));
    ...
    const ih=data.imu_health||{};
    const imuAge=(typeof ih.last_success_age_seconds==='number')?ih.last_success_age_seconds.toFixed(2)+'s':'--';
    const serialText = 'ESP32 connected=' + data.motor_bridge_connected
      + ' | TX=' + (f.serial_tx || '--')
      + ' | RX=' + (data.motor_bridge_last_line || '--')
      + ' | error=' + (data.motor_bridge_error || '--')
      + ' | IMU age=' + imuAge
      + ' | IMU errors=' + (ih.read_failure_count ?? 0);
    setText('serialDebugText', serialText);
    setText('pidOutput',f.pid_output.toFixed(1));
    setText('adrcDisturbance',(f.adrc_estimated_disturbance ?? 0).toFixed(1));
    setText('adrcVelocity',(f.adrc_estimated_velocity ?? 0).toFixed(1));
    setText('adrcOutput',(f.adrc_output ?? 0).toFixed(1));
    setText('ilcOutput',(f.ilc_output ?? 0).toFixed(1));
    setText('ilcRmse',(f.ilc_last_rmse ?? 0).toFixed(2));
...
def imu_watchdog_loop():
    """Exit on an uninterruptible I2C stall; systemd safely restarts the app."""
    while True:
        now = time.monotonic()
        reason = ""
        if initializing and imu_init_started_monotonic > 0.0:
            ...
            if age > IMU_INIT_STALL_TIMEOUT:
                reason = f"IMU initialization stalled for {age:.1f}s"
            ...
            if age > IMU_READER_STALL_TIMEOUT:
                reason = f"IMU reader stalled for {age:.1f}s"
        ...
        if reason:
            try:
                ...
                print(f"WATCHDOG: {reason}; exiting for supervised restart", flush=True)
                os._exit(70)
        time.sleep(0.2)

...
class IMURehabSystem:
    ...
    def compute_pid_output(self, target_angle, measured_angle, dt):
        ...
        derivative_error = -self.filtered_measurement_velocity
        self.derivative_error = derivative_error

        mode = str(self.controller_mode).lower()
        use_p = mode in ("p", "pi", "pid", "assist", "assist_as_needed", "aan")
        use_i = mode in ("pi", "pid") or (
            mode in ("assist", "assist_as_needed", "aan")
            and self.error_active_time >= self.assist_delay
        )
        use_d = mode in ("pid", "assist", "assist_as_needed", "aan")
    ...
    def read_frame(self):
        ...
        error, pid_output = self.compute_pid_output(
            target_angle,
            measured_angle,
            dt,
        )

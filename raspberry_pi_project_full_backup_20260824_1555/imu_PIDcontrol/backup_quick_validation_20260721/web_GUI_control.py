...
SINGLE_MOTOR_PWM_LIMIT = 30
MOTOR_CONTROL_PROFILE = "single_biceps"
...
LIVE_SINGLE_MOTOR_ALLOWED = False
...
muscle_allocator = MuscleAllocator(
    pwm_limit=SINGLE_MOTOR_PWM_LIMIT,
    active_min_pwm=0,
...
<header>
  ...
  <p>第二階段安全空跑：單一肘關節、單一二頭肌馬達命令，ESP32 僅接收 STOP</p>
</header>
<div class="container">
  <div>
    <div class="card">
    ...
    <div class="card">
      <h2>2. IMU 安裝與方向驗證</h2>
      <p class="small">本流程只驗證穩定度、漂移、方向與回零，不代表絕對角度準確度。驗證時馬達會被強制停用。</p>
      <button onclick="startValidation()">開始／重新開始驗證</button>
    ...
    <div class="card">
      ...
      <select id="targetJointInput" onchange="updateJointLabels()">
        ...
        <option value="shoulder" disabled>肩關節 shoulder（本階段鎖定）</option>
      </select>
      ...
      <p class="small">目前只做固定肘角的閉迴路命令計算。先用 60° 觀察誤差與 Desired M1；M2、M3 永遠為 0。</p>
    </div>
    ...
    <div class="card">
      ...
      <div class="grid2">
        ...
        <div><label>輸出限制（硬上限 ±30）</label><input id="outputLimitInput" value="30" max="30"></div>
    ...
    <div class="card">
      <h2>5. 馬達安全控制</h2>
      <div class="validation fail" id="dryRunBanner">單馬達 DRY RUN：僅顯示 M1 Desired PWM（±30），實際 Output 固定為 0，ESP32 只接收 STOP。</div>
      <div class="grid2">
...
function params(){return {target_joint:'elbow',target_mode:document.getElementById('targetModeInput').value,fixed_target_angle:fval('fixedTargetInput',60),trajectory_min_angle:fval('trajectoryMinInput',30),trajectory_max_angle:fval('trajectoryMaxInput',90),trajectory_period:fval('trajectoryPeriodInput',5),step_hold_time:fval('stepHoldInput',3),controller_mode:document.getElementById('controllerModeInput').value,deadband:fval('deadbandInput',3),assist_delay:fval('assistDelayInput',.25),output_limit:Math.min(Math.abs(fval('outputLimitInput',30)),30),kp:fval('kpInput',.8),ki:fval('kiInput',.02),kd:fval('kdInput',.05),integral_limit:fval('integralLimitInput',100)};}
...
function updatePage(data){
  ...
  if(data.single_motor){
    const s=data.single_motor;
    setText('dryRunBanner','單馬達 '+String(s.mode).toUpperCase()+'：'+s.active_motor
      +'，PWM 上限 ±'+s.pwm_limit+'；實際輸出 '+(s.live_output_allowed?'已允許':'固定為 0，ESP32 只接收 STOP')+'。');
...
def apply_params_to_sensor(data, reset_controller=True):
    ...
    if sensor is None:
        return
    params = {
        # Phase 2 is intentionally restricted to one elbow joint.
        "target_joint": "elbow",
        ...
        "output_limit": min(abs(safe_float(data.get("output_limit"), 30.0)), SINGLE_MOTOR_PWM_LIMIT),
    ...
    while reader_running:
        try:
            ...
            output_pwm = muscle_allocator.allocate(
                ...
                control_profile=MOTOR_CONTROL_PROFILE,
                motor_enabled=frame.get("motor_enabled", False) and LIVE_SINGLE_MOTOR_ALLOWED,
            ...
            if motor_bridge_connected and motor_bridge is not None:
                try:
                    if (not LIVE_SINGLE_MOTOR_ALLOWED
...
def init_worker(params):
    ...
    try:
        ...
        new_sensor = IMURehabSystem(
            ...
            output_limit=min(abs(safe_float(params.get("output_limit"), 30.0)), SINGLE_MOTOR_PWM_LIMIT),
...
def api_status():
    with state_lock:
        return jsonify({
            ...
            "single_motor": {
                "mode": "dry_run" if not LIVE_SINGLE_MOTOR_ALLOWED else "live",
                "control_profile": MOTOR_CONTROL_PROFILE,
                "target_joint": "elbow",
                "active_motor": "M1_biceps",
                "pwm_limit": SINGLE_MOTOR_PWM_LIMIT,
                "live_output_allowed": LIVE_SINGLE_MOTOR_ALLOWED,
                "encoder_safety_enabled": False,
                "lock_reason": (
                    "Motor 1 direction, encoder/limit inputs and physical emergency stop are not verified."
                    if not LIVE_SINGLE_MOTOR_ALLOWED else ""
                ),
            },
...
def api_set_motor():
    ...
    with state_lock:
        if sensor is None:
            return jsonify({"ok": False, "message": "尚未初始化 IMU。"})
        if enabled and not LIVE_SINGLE_MOTOR_ALLOWED:
            sensor.motor_enabled = False
            return jsonify({
                "ok": False,
                "message": "單馬達 DRY RUN：實際 PWM 輸出仍被硬鎖定；目前只顯示 M1 Desired PWM（±30），ESP32 只接收 STOP。"
            }), 403

...
class WebSafetyTests(unittest.TestCase):
    ...
    def test_home_and_status_are_available(self):
        ...
        self.assertIn("單馬達 DRY RUN", html)
        self.assertIn("42 秒快速驗證", html)
        response = self.client.get("/api/status")
        self.assertEqual(response.status_code, 200)
        status = response.get_json()
        self.assertIn("validation", status)
        self.assertEqual(status["single_motor"]["control_profile"], "single_biceps")
        self.assertEqual(status["single_motor"]["pwm_limit"], 30)
        self.assertFalse(status["single_motor"]["live_output_allowed"])
    ...
    def test_server_forces_elbow_and_clamps_requested_output(self):

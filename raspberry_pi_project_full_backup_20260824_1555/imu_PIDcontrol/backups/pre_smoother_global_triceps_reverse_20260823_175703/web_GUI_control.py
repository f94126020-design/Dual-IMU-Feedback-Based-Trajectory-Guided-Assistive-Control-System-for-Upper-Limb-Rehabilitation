  ...
  <div class="controls">
    ...
    <div class="card controller-card collapsible-card collapsed">
      ...
      <div id="feedforwardParameterPanel" class="parameter-panel controller-panel">
        ...
        <p class="small">純跟隨使用同步狀態機：馬達歸零隔離 40 ms、取樣角度 80 ms，再依取樣方向固定輸出此 PWM 200 ms。輸出期間的 IMU 不會改變或延長命令，連續階段最多 2 秒。</p>
      </div>
...
function updateControllerMode(){
  ...
  const descriptions={
    ...
    following_only:'間歇取樣跟隨：馬達關閉時取樣患者角度變化，再固定保持 PWM；40 ms 隔離＋80 ms 取樣＋200 ms 輸出，忽略目標軌跡。',
...
function updatePage(data){
  ...
  if(updateSlowUi && data.motor_output){
    ...
    setText('dryRunBanner',demo
      ...
      :followingOnly
      ?'間歇取樣跟隨 LIVE：馬達歸零 '+(((s.event_following||{}).blanking_duration_seconds||.04)*1000)+' ms＋取樣 '+(((s.event_following||{}).sample_duration_seconds||.08)*1000)+' ms＋固定 '+s.feedforward_min_pwm+' PWM 輸出 '+(((s.event_following||{}).hold_duration_seconds||.2)*1000)+' ms；連續階段最多 '+((s.event_following||{}).maximum_session_seconds||2)+' 秒。'
        ...
        return jsonify({
            ...
            "motor_output": {
                ...
                "event_following": {
                    ...
                    "sample_duration_seconds": getattr(sensor, "following_intent_confirm_time", 0.08) if sensor is not None else 0.08,

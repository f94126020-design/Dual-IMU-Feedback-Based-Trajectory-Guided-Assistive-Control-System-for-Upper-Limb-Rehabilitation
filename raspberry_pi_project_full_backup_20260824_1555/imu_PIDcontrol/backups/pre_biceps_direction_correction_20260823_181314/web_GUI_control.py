  ...
  <div class="controls">
    ...
    <div class="card controller-card collapsible-card collapsed">
      ...
      <div id="feedforwardParameterPanel" class="parameter-panel controller-panel">
        ...
        <label>取樣後固定輸出 PWM</label><input id="feedforwardMinInput" value="10" min="0" max="255">
        <p class="small">純跟隨使用同步狀態機：馬達歸零隔離 40 ms、取樣角度 60 ms，再依取樣方向固定輸出此 PWM 160 ms。輸出期間的 IMU 不會改變或延長命令，連續階段最多 2 秒。</p>
      </div>
...
function updateControllerMode(){
  ...
  const descriptions={
    ...
    following_only:'間歇取樣跟隨：馬達關閉時取樣患者角度變化，再固定保持 PWM；40 ms 隔離＋60 ms 取樣＋160 ms 輸出，忽略目標軌跡。',
...
function updatePage(data){
  ...
  if(updateSlowUi && data.motor_output){
    ...
    setText('dryRunBanner',demo
      ...
      :followingOnly
      ?'間歇取樣跟隨 LIVE：馬達歸零 '+(((s.event_following||{}).blanking_duration_seconds||.04)*1000)+' ms＋取樣 '+(((s.event_following||{}).sample_duration_seconds||.06)*1000)+' ms＋固定 '+s.feedforward_min_pwm+' PWM 輸出 '+(((s.event_following||{}).hold_duration_seconds||.16)*1000)+' ms；連續階段最多 '+((s.event_following||{}).maximum_session_seconds||2)+' 秒。'
...
def send_motor_pwm_with_locks(pwm_biceps, pwm_triceps, pwm_deltoid):
    ...
    physical_command = apply_motor_direction_sign(locked_command)
        ...
        return jsonify({
            ...
            "motor_output": {
                ...
                "event_following": {
                    ...
                    "sample_duration_seconds": getattr(sensor, "following_intent_confirm_time", 0.06) if sensor is not None else 0.06,
                    "minimum_angle_degrees": getattr(sensor, "following_intent_min_angle", 0.4) if sensor is not None else 0.4,
                    "hold_duration_seconds": getattr(sensor, "following_pulse_duration", 0.16) if sensor is not None else 0.16,
                    ...
                    "maximum_session_seconds": getattr(sensor, "following_max_session_duration", 2.0) if sensor is not None else 2.0,

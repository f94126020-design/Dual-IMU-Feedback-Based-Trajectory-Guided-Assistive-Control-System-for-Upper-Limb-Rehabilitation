  ...
  <div class="controls">
    ...
    <div class="card safety-card collapsible-card collapsed">
      ...
      <div class="parameter-panel active">
        ...
        <div class="validation warning">僅用於整理或捲回繩索。請先 Disable Motor，確認無人體負載且急停可立即操作。同時只允許一顆馬達動作；放開按鈕、失去視窗焦點或通訊中斷會自動 STOP。</div>
        <div class="grid2">
        ...
        <div class="grid3">
          <div><label>二頭肌 Motor 1</label><button class="success jog-button" onpointerdown="startMotorJog(event,'biceps',1)">按住正轉＋</button><button class="secondary jog-button" onpointerdown="startMotorJog(event,'biceps',-1)">按住反轉－</button></div>
          <div><label>三頭肌 Motor 2</label><button class="success jog-button" onpointerdown="startMotorJog(event,'triceps',1)">按住正轉＋</button><button class="secondary jog-button" onpointerdown="startMotorJog(event,'triceps',-1)">按住反轉－</button></div>
          <div><label>三角肌 Motor 3</label><button class="success jog-button" onpointerdown="startMotorJog(event,'deltoid',1)">按住正轉＋</button><button class="secondary jog-button" onpointerdown="startMotorJog(event,'deltoid',-1)">按住反轉－</button></div>
        </div>
...
def api_motor_jog_start():
    ...
    with state_lock:
        if sensor is None or not initialized:
            return jsonify({"ok": False, "message": "請先初始化系統，再使用單顆馬達點動。"}), 400
        if sensor.motor_enabled:

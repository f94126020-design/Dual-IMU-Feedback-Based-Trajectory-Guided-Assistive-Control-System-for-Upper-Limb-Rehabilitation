...
class ProfileBControlTests(unittest.TestCase):
    ...
    def test_event_following_uses_bounded_negative_velocity_feedback(self):
        system = IMURehabSystem(
            controller_mode="following_only",
        ...
        self.assertEqual(system.compute_event_following_output("elbow", 0.0, 12.0, 0.00), 0.0)
        self.assertEqual(system.event_following_status("elbow")["state"], "confirming")
        self.assertEqual(system.compute_event_following_output("elbow", 0.6, 12.0, 0.05), 0.0)
        self.assertEqual(system.compute_event_following_output("elbow", 1.2, 12.0, 0.10), 37.5)
        self.assertEqual(system.event_following_status("elbow")["state"], "assist")

        # Motor-driven speed reduces rather than reinforces the PWM.
        self.assertEqual(system.compute_event_following_output("elbow", 8.0, 100.0, 0.20), 0.0)
        self.assertGreater(system.compute_event_following_output("elbow", 13.0, 10.0, 0.50), 0.0)
        # Motion cannot extend the bounded 1.2-second assist window.
        self.assertEqual(system.compute_event_following_output("elbow", 20.0, 10.0, 1.31), 0.0)
        self.assertEqual(system.event_following_status("elbow")["state"], "blanking")
        self.assertEqual(system.compute_event_following_output("elbow", 22.0, 100.0, 1.42), 0.0)
        self.assertEqual(system.event_following_status("elbow")["state"], "rearm")

        # Persistent motor/person motion cannot retrigger.  A quiet window is
        # mandatory before a new opposite-direction intent can be confirmed.
        self.assertEqual(system.compute_event_following_output("elbow", 25.0, 100.0, 1.60), 0.0)
        self.assertEqual(system.compute_event_following_output("elbow", 25.0, 0.0, 1.61), 0.0)
        self.assertEqual(system.compute_event_following_output("elbow", 25.0, 0.0, 1.72), 0.0)
        self.assertEqual(system.event_following_status("elbow")["state"], "idle")
        self.assertEqual(system.compute_event_following_output("elbow", 25.0, -12.0, 1.73), 0.0)
        self.assertEqual(system.compute_event_following_output("elbow", 23.7, -12.0, 1.84), -37.5)

    def test_event_following_is_disarmed_and_reset_while_motor_disabled(self):
        system = IMURehabSystem(controller_mode="following_only")
        self.assertEqual(system.compute_event_following_output("shoulder", 0.0, 50.0, 0.0), 0.0)
        self.assertEqual(system.event_following_status("shoulder")["state"], "idle")
        system.motor_enabled = True
        system.compute_event_following_output("shoulder", 0.0, 12.0, 0.1)
        self.assertEqual(system.event_following_status("shoulder")["state"], "confirming")
        system.motor_enabled = False
        self.assertEqual(system.compute_event_following_output("shoulder", 2.0, 30.0, 0.2), 0.0)
        self.assertEqual(system.event_following_status("shoulder")["state"], "idle")

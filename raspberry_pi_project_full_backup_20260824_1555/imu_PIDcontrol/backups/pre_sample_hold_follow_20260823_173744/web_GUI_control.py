  ...
  <div class="controls">
    ...
    <div class="card controller-card collapsible-card collapsed">
      ...
      <select id="controllerModeInput">
        ...
        <option value="following_only">意圖觸發速度跟隨（無目標、無軌跡）</option>
      ...
      <div id="feedforwardParameterPanel" class="parameter-panel controller-panel">
        ...
        <label>速度跟隨最大 PWM</label><input id="feedforwardMinInput" value="10" min="0" max="255">
        <p class="small">純跟隨只在馬達為零時確認主動動作；助力期間以角速度負回授在 0 到此上限間調整，速度越接近參考值，PWM 越小。單次助力最長 1.2 秒且不會被動作延長。</p>
      </div>
    ...
    <div class="card safety-card collapsible-card collapsed">
      ...
      <div class="parameter-panel active">
        ...
        <div class="validation warning">僅用於初始化前整理或捲回繩索，不需要先校正 IMU。請確認主控制未啟用、無人體負載且急停可立即操作。同時只允許一顆馬達動作；放開按鈕、失去視窗焦點或通訊中斷會自動 STOP。各馬達安全鎖是全系統鎖：鎖定後，該實體馬達在手動點動及所有復健／測試模式都固定輸出 0。</div>
        <div class="grid2">
...
function updateControllerMode(){
  ...
  const descriptions={
    ...
    following_only:'意圖觸發速度跟隨：先確認主動動作並鎖定方向，再以角速度負回授提供最長 1.2 秒的助力；忽略目標軌跡。',
...
function updatePage(data){
  ...
  if(updateSlowUi && data.motor_output){
    ...
    setText('dryRunBanner',demo
      ...
      :followingOnly
      ?'意圖觸發速度跟隨 LIVE：確認主動動作後鎖定方向，以速度負回授調整 0～'+s.feedforward_min_pwm+' PWM；單次最長 '+((s.event_following||{}).pulse_duration_seconds||1.2)+' 秒，且不會由馬達動作延長。'
    ...
    const lockSummary=lockedNames.length
      ?'\n全系統馬達鎖定中：'+lockedNames.join('、')+'（所有模式固定輸出 0）'
      :'\n全系統馬達鎖：三顆皆未鎖定。';
    setText('motorInterlockText',(s.patient_rom_active
      ?'個人化 ROM 邊界保護已啟用：接近端點會漸降收線 PWM，到達端點後禁止繼續往外收線，仍允許反向離開。'
      :'尚未套用個人化 ROM；目前只有 PWM、急停與通訊逾時保護。')+lockSummary);
  }
  ...
  if(data.frame){
    ...
    const pulseStateLabels={idle:'等待動作',confirming:'確認意圖',assist:'速度助力',blanking:'隔離中',rearm:'等待靜止'};
    let motionText=f.rom_limit_active ? f.motion_state+'｜ROM 邊界限制中' : f.motion_state;
    if(followingOnly){motionText+='｜肘：'+(pulseStateLabels[f.elbow_following_state]||f.elbow_following_state||'--')+'｜肩：'+(pulseStateLabels[f.shoulder_following_state]||f.shoulder_following_state||'--');}
...
def send_motor_pwm_with_locks(pwm_biceps, pwm_triceps, pwm_deltoid):
    ...
    command = apply_motor_channel_locks(
        MusclePWM(pwm_biceps, pwm_triceps, pwm_deltoid)
    )
    motor_bridge.set_pwm(command.biceps, command.triceps, command.deltoid)
    return command
    ...
    with state_lock:
        return jsonify({
            ...
            "motor_output": {
                ...
                "event_following": {
                    "intent_velocity_threshold": getattr(sensor, "following_intent_velocity_threshold", 10.0) if sensor is not None else 10.0,
                    "confirm_time_seconds": getattr(sensor, "following_intent_confirm_time", 0.10) if sensor is not None else 0.10,
                    "minimum_angle_degrees": getattr(sensor, "following_intent_min_angle", 1.0) if sensor is not None else 1.0,
                    "pulse_duration_seconds": getattr(sensor, "following_pulse_duration", 1.20) if sensor is not None else 1.20,
                    "blanking_duration_seconds": getattr(sensor, "following_blanking_duration", 0.10) if sensor is not None else 0.10,
                    ...
                    "velocity_margin": getattr(sensor, "following_velocity_margin", 15.0) if sensor is not None else 15.0,
                    "velocity_control_band": getattr(sensor, "following_velocity_control_band", 20.0) if sensor is not None else 20.0,
                },

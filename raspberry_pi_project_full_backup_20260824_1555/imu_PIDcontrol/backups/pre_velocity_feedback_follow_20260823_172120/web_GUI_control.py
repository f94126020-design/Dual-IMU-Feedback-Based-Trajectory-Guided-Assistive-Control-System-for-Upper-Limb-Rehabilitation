  ...
  <div class="controls">
    ...
    <div class="card controller-card collapsible-card collapsed">
      ...
      <select id="controllerModeInput">
        ...
        <option value="following_only">意圖觸發脈衝跟隨（無目標、無補償）</option>
      ...
      <div id="feedforwardParameterPanel" class="parameter-panel controller-panel">
        ...
        <label>偵測到動作時的固定基礎跟隨 PWM（設定多少就輸出多少）</label><input id="feedforwardMinInput" value="10" min="0" max="255">
        <p class="small">純跟隨只在馬達為零時確認主動動作，確認後輸出一次固定 PWM 脈衝，再等待動作停止才重新允許觸發；脈衝不會被馬達造成的角速度延長。</p>
      </div>
...
function updateControllerMode(){
  ...
  const descriptions={
    ...
    following_only:'意圖觸發脈衝跟隨：馬達為零時確認主動動作，輸出定時脈衝後等待靜止再重新觸發；忽略目標軌跡且不使用控制器補償。',
...
function updatePage(data){
  ...
  if(updateSlowUi && data.motor_output){
    ...
    setText('dryRunBanner',demo
      ...
      :followingOnly
      ?'意圖觸發脈衝跟隨 LIVE：馬達為零時確認動作，固定輸出 150 ms，隔離 200 ms，等動作靜止後才重新觸發；控制器補償固定為 0。'
  ...
  if(data.frame){
    ...
    const pulseStateLabels={idle:'等待動作',confirming:'確認意圖',assist:'輔助脈衝',blanking:'隔離中',rearm:'等待靜止'};
        ...
        return jsonify({
            ...
            "motor_output": {
                ...
                "event_following": {
                    ...
                    "pulse_duration_seconds": getattr(sensor, "following_pulse_duration", 0.15) if sensor is not None else 0.15,
                    "blanking_duration_seconds": getattr(sensor, "following_blanking_duration", 0.20) if sensor is not None else 0.20,

...
class ProfileBControlTests(unittest.TestCase):
    ...
    def test_event_following_uses_one_fixed_pulse_and_cannot_self_extend(self):
        system = IMURehabSystem(
            controller_mode="following_only",
        ...
        self.assertEqual(system.compute_event_following_output("elbow", 1.2, 12.0, 0.10), 50.0)
        self.assertEqual(system.event_following_status("elbow")["state"], "assist")

        # Even an extreme same-direction velocity cannot lengthen the pulse.
        self.assertEqual(system.compute_event_following_output("elbow", 8.0, 100.0, 0.20), 50.0)
        self.assertEqual(system.compute_event_following_output("elbow", 13.0, 100.0, 0.251), 0.0)
        self.assertEqual(system.event_following_status("elbow")["state"], "blanking")
        self.assertEqual(system.compute_event_following_output("elbow", 20.0, 100.0, 0.46), 0.0)
        ...
        self.assertEqual(system.compute_event_following_output("elbow", 25.0, 100.0, 0.70), 0.0)
        self.assertEqual(system.compute_event_following_output("elbow", 25.0, 0.0, 0.71), 0.0)
        self.assertEqual(system.compute_event_following_output("elbow", 25.0, 0.0, 0.82), 0.0)
        self.assertEqual(system.event_following_status("elbow")["state"], "idle")
        self.assertEqual(system.compute_event_following_output("elbow", 25.0, -12.0, 0.83), 0.0)
        self.assertEqual(system.compute_event_following_output("elbow", 23.7, -12.0, 0.94), -50.0)

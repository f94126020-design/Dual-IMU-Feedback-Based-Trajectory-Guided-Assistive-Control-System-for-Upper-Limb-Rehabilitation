...
class IMURehabSystem:
    ...
    def __init__(
        ...
        following_pulse_duration=0.15,
        following_blanking_duration=0.20,
    ...
    def compute_event_following_output(self, joint, measured_angle, measured_velocity, now):
        """One-shot intent assistance used only by ``following_only``.

        Intent is confirmed while the command is zero.  Once confirmed, the
        direction is latched and a fixed-duration GUI-PWM pulse is emitted.
        IMU motion during the pulse cannot extend it.  Afterward a blanking
        interval and a stationary re-arm are required, preventing motor-driven
        motion from continuously retriggering itself.
        """
        ...
        if phase == "confirming":
            ...
            if elapsed >= self.following_intent_confirm_time:
                if (
                    ...
                    return direction * self.assist_feedforward_min_pwm
                state.update({
                    "state": "idle", "direction": 0.0,
                    "state_started": None, "samples": 0,
                    "consistent_samples": 0, "quiet_started": None,
        ...
        if phase == "assist":
            if timestamp - state["state_started"] < self.following_pulse_duration:
                return state["direction"] * self.assist_feedforward_min_pwm

...
<head>
  ...
  <style>
    ...
    .controls{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));grid-template-areas:"init task" "controller safety" "rom rom" "frequency frequency" "record record";gap:12px;align-items:start}
    .card{background:white;border-radius:12px;padding:16px;box-shadow:0 2px 8px rgba(0,0,0,.08);margin:0}.card h2{font-size:18px;margin:0 0 12px}.init-card{grid-area:init}.rom-card{grid-area:rom}.task-card{grid-area:task}.controller-card{grid-area:controller}.safety-card{grid-area:safety}.frequency-card{grid-area:frequency}.record-card{grid-area:record}
    .monitor-column{position:sticky;top:12px;align-self:start;display:grid;gap:12px}.monitor-column .card{padding:12px}.live-card .metrics{grid-template-columns:repeat(6,minmax(78px,1fr));gap:7px}.live-card .metric{padding:8px}.live-card .metric-title{font-size:11px}.live-card .metric-value{font-size:18px;margin-top:4px}.chart-card h2{font-size:20px}.chart-card .small{margin-bottom:0}
    .controls .card{padding:13px}.controls .card h2{margin-bottom:9px}.controls label{margin:5px 0 3px}.controls input,.controls select{padding:7px}.controls button{padding:9px;margin:4px 0}.controls .parameter-panel{margin-top:8px;padding:10px}.controls .validation{padding:8px;margin:6px 0;line-height:1.4}.record-card .record-fields{grid-template-columns:repeat(3,minmax(0,1fr))}.record-card .record-actions{display:grid;grid-template-columns:1fr 1fr;gap:8px}.record-card #recordingState{margin:8px 0 2px}
    ...
    @media(max-width:820px){.controls{grid-template-columns:1fr;grid-template-areas:"init" "task" "controller" "safety" "rom" "frequency" "record"}.live-card .metrics{grid-template-columns:repeat(2,1fr)}.record-card .record-fields{grid-template-columns:1fr}#angleCanvas{height:520px}}
  </style>
</head>
<body>
<header>
  ...
  <div class="controls">
    ...
    <div class="card controller-card collapsible-card collapsed">
      ...
      <select id="controllerModeInput">
        ...
        <option value="following_only">純 IMU 跟隨（無目標、無補償）</option>
      ...
      <div id="controllerCommonPanel" class="parameter-panel active">
        ...
        <label>大幅落後補償上限 PWM</label><input id="outputLimitInput" value="55" min="0" max="55">
      </div>
      <div id="pidParameterPanel" class="parameter-panel controller-panel">
        <div class="parameter-title">PID 回授參數</div>
        <div class="grid2">
          <div><label>Kp</label><input id="kpInput" value="5"></div>
          ...
          <div><label>落後補償門檻 度</label><input id="deadbandInput" value="4" min="3" max="5" step="0.5"></div>
        </div>
      </div>
      <div id="feedforwardParameterPanel" class="parameter-panel controller-panel">
        <div class="parameter-title">IMU 動作跟隨模式</div>
        <div class="grid2">
          <div><label>實際角速度增益 PWM/(°/s)</label><input id="feedforwardGainInput" value="0.35"></div>
          <div><label>偵測到動作時跟隨 PWM</label><input id="feedforwardMinInput" value="200" min="0" max="255"></div>
        </div>
      </div>
      ...
      <div id="demoParameterPanel" class="parameter-panel controller-panel">
        ...
        <label>展示模式 PWM 上限（最高 100）</label><input id="demoOutputLimitInput" value="100" min="0" max="100">
      </div>
    ...
    <div class="card safety-card collapsible-card collapsed">
      <h2>5. 馬達安全控制</h2>
      <div class="validation warning" id="dryRunBanner">IMU 動作跟隨 LIVE：三條肌腱依病患實際動作收／放線；大幅落後時才由所選控制器增加助力。請先確認馬達固定、繩索未連接人體且急停可用。</div>
      <div class="grid2">
      ...
      <div class="parameter-panel active">
        ...
        <div class="validation warning">僅用於初始化前整理或捲回繩索，不需要先校正 IMU。請確認主控制未啟用、無人體負載且急停可立即操作。同時只允許一顆馬達動作；放開按鈕、失去視窗焦點或通訊中斷會自動 STOP。各馬達安全鎖只限制本區的手動點動，不影響正式控制模式。</div>
        <div class="grid2">
        ...
        <div class="grid3">
          <div><label>二頭肌 Motor 1</label><label><input id="motorJogLockBiceps" class="motor-jog-lock" type="checkbox" style="width:auto" onchange="setMotorJogLock('biceps',this.checked)"> 鎖定二頭肌點動</label><button class="success jog-button" data-jog-motor="biceps" onpointerdown="startMotorJog(event,'biceps',1)">按住正轉＋</button><button class="secondary jog-button" data-jog-motor="biceps" onpointerdown="startMotorJog(event,'biceps',-1)">按住反轉－</button></div>
          <div><label>三頭肌 Motor 2</label><label><input id="motorJogLockTriceps" class="motor-jog-lock" type="checkbox" style="width:auto" onchange="setMotorJogLock('triceps',this.checked)"> 鎖定三頭肌點動</label><button class="success jog-button" data-jog-motor="triceps" onpointerdown="startMotorJog(event,'triceps',1)">按住正轉＋</button><button class="secondary jog-button" data-jog-motor="triceps" onpointerdown="startMotorJog(event,'triceps',-1)">按住反轉－</button></div>
          <div><label>三角肌 Motor 3</label><label><input id="motorJogLockDeltoid" class="motor-jog-lock" type="checkbox" style="width:auto" onchange="setMotorJogLock('deltoid',this.checked)"> 鎖定三角肌點動</label><button class="success jog-button" data-jog-motor="deltoid" onpointerdown="startMotorJog(event,'deltoid',1)">按住正轉＋</button><button class="secondary jog-button" data-jog-motor="deltoid" onpointerdown="startMotorJog(event,'deltoid',-1)">按住反轉－</button></div>
        </div>
...
const persistentParameterIds=[
  ...
  'outputLimitInput','deadbandInput','feedforwardGainInput','feedforwardMinInput',
...
persistentParameterIds.forEach(id=>{
  const element=document.getElementById(id);
  if(element)element.addEventListener('change',saveParameterInputs);
});
...
function updateControllerMode(){
  ...
  document.getElementById('targetJointInput').disabled=followingOnly;
  ...
  const descriptions={
    ...
    following_only:'純 IMU 跟隨：三顆馬達只依實際肘／肩動作收放線，完全忽略目標軌跡且不使用控制器補償。',
...
function clearMotorJogUi(message='點動待機｜預設 100 PWM'){
  ...
  motorJogHeartbeatBusy=false;motorJogActive=false;
...
async function setMotorJogLock(motor,locked){
  ...
  const res=await postJSON('/api/motor_jog/locks',{motor,locked});
...
async function stopMotorJog(showToast=false){
  motorJogHeld=false;clearInterval(motorJogHeartbeatTimer);motorJogHeartbeatTimer=null;
...
window.addEventListener('pagehide',()=>{if(motorJogHeld||motorJogActive)stopMotorJog(false);});
document.addEventListener('visibilitychange',()=>{if(document.hidden&&(motorJogHeld||motorJogActive))stopMotorJog(false);});
document.querySelectorAll('.jog-button').forEach(button=>button.addEventListener('pointerleave',()=>{if(motorJogHeld||motorJogActive)stopMotorJog(false);}));
...
function updatePage(data){
  ...
  if(updateSlowUi){
    ...
    updateMotorJogLocks((data.motor_jog||{}).locks||{});
  }
  if(updateSlowUi && data.motor_output){
    ...
    setText('dryRunBanner',demo
      ...
      :followingOnly
      ?'純 IMU 跟隨 LIVE：三顆馬達只依病患實際動作收／放線，完全忽略目標軌跡，控制器補償固定為 0。'
      :'IMU 動作跟隨 LIVE：實際角速度收放線＋大幅落後補償；三頭放線比 '+s.triceps_release_ratio+'、二頭放線比 '+s.biceps_release_ratio+'、低通 '+s.command_filter_tau+'s、換向停頓 '+Math.round(s.reverse_deadtime*1000)+'ms；按鍵確認及 ESP32 ARM 成功後才輸出。');
    setText('motorInterlockText',s.patient_rom_active
      ?'個人化 ROM 邊界保護已啟用：接近端點會漸降收線 PWM，到達端點後禁止繼續往外收線，仍允許反向離開。'
      :'尚未套用個人化 ROM；目前只有 PWM、急停與通訊逾時保護。');
  }
  ...
  if(data.frame){
    ...
    setText('targetAngle',followingOnly?'忽略':f.target_angle.toFixed(1)+'°');
    setText('errorValue',followingOnly?'無補償':f.error.toFixed(1)+'°');
    setText('motionState',f.rom_limit_active ? f.motion_state+'｜ROM 邊界限制中' : f.motion_state);
...
function drawPlot(){
  ...
  ctx.fillStyle='#6b7280';ctx.fillText(followingOnly?'Target ignored':'Target preview',ml+190,topMt+18);
...
async function pollStatus(){
  ...
  setTimeout(pollStatus,document.hidden?1000:livePollDelayMs);
}
...
def apply_params_to_sensor(data, reset_controller=True):
    ...
    params = {
        ...
        "assist_feedforward_min_pwm": min(max(0.0, safe_float(data.get("assist_feedforward_min_pwm"), 200.0)), 255.0),
        ...
        "output_limit": min(abs(safe_float(data.get("output_limit"), 55.0)), 55.0),
        ...
        try:
            ...
            if therapeutic_following:
                ...
                elbow_pwm = muscle_allocator.allocate(
                    ...
                    # Inside the ROM soft zone the IMU layer deliberately
                    # scales PWM below the therapeutic minimum.  Reapplying
                    # the minimum here would defeat the boundary fade.
                    enforce_minimum=not frame.get("elbow_rom_limit", {}).get("limited", False),
                    feedback=fb,
                    now=frame.get("time"),
                )
                shoulder_pwm = muscle_allocator.allocate(
                    ...
                    enforce_minimum=not frame.get("shoulder_rom_limit", {}).get("limited", False),
                    feedback=fb,
                    now=frame.get("time"),
                )
...
def init_worker(params):
    ...
    try:
        ...
        new_sensor = IMURehabSystem(
            ...
            assist_feedforward_min_pwm=min(max(0.0, safe_float(params.get("assist_feedforward_min_pwm"), 200.0)), 255.0),
            assist_feedforward_velocity_threshold=2.0,
            ...
            output_limit=min(abs(safe_float(params.get("output_limit"), 55.0)), 55.0),
    ...
    with state_lock:
        return jsonify({
            ...
            "motor_output": {
                ...
                "feedforward_gain": getattr(sensor, "assist_feedforward_gain", 0.35) if sensor is not None else 0.35,
                "feedforward_min_pwm": getattr(sensor, "assist_feedforward_min_pwm", 200.0) if sensor is not None else 200.0,
...
@app.route("/api/motor_jog/locks", methods=["POST"])
def api_motor_jog_locks():
    global motor_test_running, motor_jog_active, motor_jog_session
    data = request.get_json(force=True) or {}
    motor = str(data.get("motor", "")).lower()
    if motor not in motor_jog_locks:
        return jsonify({"ok": False, "message": "馬達通道無效。", "locks": dict(motor_jog_locks)}), 400
    locked = safe_bool(data.get("locked", False))
    with state_lock:
        motor_jog_locks[motor] = locked
        stop_required = bool(
    ...
    message = f"{labels[motor]}手動點動{action}{suffix}。"
    set_status(message)
    return jsonify({"ok": True, "message": message, "locks": dict(motor_jog_locks)})
...
def api_motor_jog_start():
    ...
    with state_lock:
        if motor_jog_locks.get(motor, False):
            return jsonify({
                "ok": False,
                "message": f"{motor} 手動點動已鎖定，請先解除該馬達安全鎖。",
                "locks": dict(motor_jog_locks),
            }), 409

...
class IMUValidationSession:
    ...
    RETURN_ERROR_LIMIT = 8.0
    MIN_EXCURSION = 20.0
    DIRECTION_AGREEMENT_MIN = 0.80
    PLANE_ACCURACY_MIN = 0.80
    PLANE_SEPARATION_DEG_MIN = 20.0
    ...
    def _static_metric(self, samples, key):
        ...
        first = [s[key] for s in samples if s["elapsed"] < 5.0]
        last = [s[key] for s in samples if s["elapsed"] >= 25.0]
        return {
    ...
    def _analyze_static(self, samples):
        enough = len(samples) >= 100 and not self._has_data_gaps(samples)
        metrics = {
    ...
    def _cycle_windows(self, samples):
        ...
        for index in range(self.REQUIRED_CYCLES):
            base = index * 10.0
            cycles.append({
                "out": self._window(samples, base, base + 3.0),
                "hold": self._window(samples, base + 3.0, base + 5.0),
                "return": self._window(samples, base + 5.0, base + 8.0),
                "neutral": self._window(samples, base + 8.0, base + 10.0),
            })
        return cycles
    ...
    def _direction_agreement(self, windows, key, sign):
        ...
        for window, expected in windows:
            for a, b in zip(window, window[1:]):
                ...
                if dt <= 0.0:
                    continue
                velocity = (b[key] - a[key]) / dt
                if abs(velocity) < 1.0:
                    continue
                active += 1
                if velocity * sign * expected > 0.0:
                    correct += 1
    ...
    def _motion_metrics(self, samples, key, baseline, sign=1.0):
        ...
        for cycle in cycles:
            if min(len(cycle[name]) for name in ("out", "hold", "return", "neutral")) < 2:
                continue
            complete_cycles += 1
            hold = self._mean([s[key] for s in cycle["hold"]])
            neutral = self._mean([s[key] for s in cycle["neutral"]])
            amplitudes.append(abs((hold - baseline) * sign))
    ...
    def _analyze_elbow(self, samples):
        ...
        passed = (
            ...
            and all(v >= self.MIN_EXCURSION for v in metric["amplitudes"])
        ...
        if metric["complete_cycles"] != self.REQUIRED_CYCLES:
            reasons.append("未完成五次肘屈伸")
        if metric["amplitudes"] and any(v < self.MIN_EXCURSION for v in metric["amplitudes"]):
            reasons.append("至少一次肘屈曲幅度不足 20°")
        if metric["max_return_error"] > self.RETURN_ERROR_LIMIT:
            reasons.append("肘關節回零誤差超過 8°")
        if metric["direction_agreement"] < self.DIRECTION_AGREEMENT_MIN:
            reasons.append("肘屈伸方向一致率低於 80%")
    ...
    def _analyze_shoulder(self, stage, samples):
        ...
        passed = (
            ...
            and all(v >= self.MIN_EXCURSION for v in metrics["amplitudes"])
        ...
        if metrics["complete_cycles"] != self.REQUIRED_CYCLES:
            reasons.append(f"未完成五次肩關節{label}")
        if metrics["amplitudes"] and any(v < self.MIN_EXCURSION for v in metrics["amplitudes"]):
            reasons.append(f"至少一次{label}幅度不足 20°")
        if metrics["max_return_error"] > self.RETURN_ERROR_LIMIT:
            reasons.append(f"{label}回零誤差超過 8°")
        if metrics["direction_agreement"] < self.DIRECTION_AGREEMENT_MIN:
            reasons.append(f"{label}抬舉／下放方向一致率低於 80%")
        if self._has_data_gaps(samples):
            reasons.append("測試期間 IMU 資料中斷")
        return {
            "passed": passed,
    ...
    def _phase_at(self, stage, elapsed):
        ...
        cycle = min(int(elapsed // 10.0) + 1, self.REQUIRED_CYCLES)
        phase_time = elapsed % 10.0
        ...
        if phase_time < 3.0:
            ...
            action = "保持回零"
        return f"第 {cycle}/5 次：{joint}{action}", cycle

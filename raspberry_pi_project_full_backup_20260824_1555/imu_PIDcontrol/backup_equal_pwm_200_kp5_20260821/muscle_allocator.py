    ...
    def allocate(
        ...
        if target_joint == "elbow":
            if u > 0:
                ...
                pwm.biceps = self.apply_min_pwm(u, 0)
                pwm.triceps = self.apply_min_pwm(
                    -int(abs(u) * self.antagonist_release_gain), 1
                )
            elif u < 0:
                # elbow extension assistance: triceps winds, biceps releases
                pwm.biceps = self.apply_min_pwm(
                    -int(abs(u) * self.antagonist_release_gain), 0
                )
                pwm.triceps = self.apply_min_pwm(abs(u), 1)
            pwm = self.apply_cable_return_compensation(pwm, abs(u))

        elif target_joint == "shoulder":
            if u > 0:
                pwm.deltoid = self.apply_min_pwm(u, 2)
            elif u < 0:
                pwm.deltoid = (
                    self.apply_min_pwm(u, 2)

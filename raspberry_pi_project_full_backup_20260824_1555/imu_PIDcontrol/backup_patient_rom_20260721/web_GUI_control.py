...
<div class="container">
  <div>
    ...
    <div class="card">
      <h2>2. IMU 安裝與方向驗證</h2>
      <p class="small">一分鐘快速驗證：42 秒有效量測，加上每階段 3 秒準備倒數。只驗證穩定度、方向與回零，不代表絕對角度準確度。</p>
      <button onclick="startValidation()">開始／重新開始一分鐘快速驗證</button>
      <button id="validationStageButton" class="secondary" onclick="prepareAndStartValidationStage()" disabled>3 秒準備後開始目前階段</button>
      ...
      <div class="protocol" id="validationProtocol">按下快速驗證後，這裡會顯示每個階段的完整姿勢、節奏與通過門檻。</div>
...
const validationStageDetails={
  static:{
    title:'靜止檢查｜6 秒',
    body:'姿勢：站直或坐直，手臂自然下垂。\n肘關節：完全伸直。前臂：中立，手掌朝向身體。\n動作：全程不要動、不要聳肩、不要碰觸 IMU。\n通過門檻：角度標準差 ≤ 2.5°，前後漂移 ≤ 4°。'
  },
  elbow:{
    title:'肘關節屈伸｜2 次，每次 6 秒',
    body:'固定：上臂貼近身體，肩膀與軀幹不要跟著動。只動前臂。\n0–2 秒：平順彎曲手肘，建議至少 30°。\n2–3 秒：保持彎曲，不要晃動。\n3–5 秒：平順伸直手肘。\n5–6 秒：保持完全伸直，等待下一次。\n通過門檻：每次幅度 ≥ 15°、回零誤差 ≤ 15°、方向正確率 ≥ 75%。'
  },
  shoulder_front:{
    title:'肩關節前平舉｜2 次，每次 6 秒',
    body:'起始：手臂自然下垂，肘保持伸直，手掌朝內。\n0–2 秒：手臂正前方抬起，建議至少 30°；不要彎肘、聳肩或後仰。\n2–3 秒：停在前方保持。\n3–5 秒：沿原路放回身側。\n5–6 秒：保持自然下垂。\n通過門檻：每次幅度 ≥ 8°、回零誤差 ≤ 15°、方向正確率 ≥ 75%。'
  },
  shoulder_side:{
    title:'肩關節側平舉｜2 次，每次 6 秒',
    body:'起始：手臂自然下垂，肘保持伸直，身體不要側傾。\n0–2 秒：手臂從身體正側方抬起，建議至少 30°；不要往前帶、聳肩或彎肘。\n2–3 秒：停在側方保持。\n3–5 秒：沿原路放回身側。\n5–6 秒：保持自然下垂。\n通過門檻：幅度 ≥ 8°、回零 ≤ 15°、方向 ≥ 75%；前舉／側舉特徵分離 ≥ 12°、分類 ≥ 75%。'
  }
};
...
async function prepareAndStartValidationStage(){
  ...
  const stage=(data.validation||{}).expected_stage;
  if(!stage)return;
...
function updateValidationPanel(v){
  ...
  if(v.current_stage){
    ...
    const duration=(v.current_stage==='static')?6:12;

...
class ProfileBControlTests(unittest.TestCase):
    def test_channel_read_error_names_the_disconnected_imu(self):
        system = IMURehabSystem(channels=(0, 1))
        system.bus = object()
        system.select_channel = mock.Mock()
        system.get_raw_data = mock.Mock(side_effect=OSError("Remote I/O error"))

        with self.assertRaisesRegex(RuntimeError, "前臂 IMU.*Channel 1"):
            system.read_channel_raw(1, retries=2)

        status = system.imu_channel_status()
        self.assertFalse(status["forearm"]["connected"])
        self.assertTrue(status["forearm"]["checked"])
        self.assertEqual(status["forearm"]["failure_count"], 1)
        self.assertIn("Remote I/O error", status["forearm"]["last_error"])

    def test_scan_error_lists_the_missing_required_imu(self):
        system = IMURehabSystem(channels=(0, 1))
        system.bus = object()
        system.select_channel = mock.Mock()
        system.init_mpu = mock.Mock(side_effect=[None, OSError("no response")])

        with self.assertRaisesRegex(RuntimeError, "前臂 IMU.*Channel 1"):
            system.scan_and_init()

        status = system.imu_channel_status()
        self.assertTrue(status["upper_arm"]["connected"])
        self.assertFalse(status["forearm"]["connected"])
    ...
    def test_elbow_cable_balance_preserves_nominal_ratio_without_debt(self):
        allocator = MuscleAllocator(enable_conditioning=False)
        nominal = MusclePWM(150, -75, 0)
        self.assertEqual(
            allocator.apply_elbow_cable_balance(nominal, 150),
            nominal,
        )
        self.assertEqual(
            allocator.elbow_cable_balance_status(),
            {
    ...
    def test_elbow_cable_balance_gradually_adds_only_needed_payout(self):
        allocator = MuscleAllocator(
            cable_return_horizon=2.0,
            enable_conditioning=False,
        )
        allocator.virtual_cable_effort["triceps"] = 20.0
        corrected = allocator.apply_elbow_cable_balance(
            MusclePWM(150, -75, 0),
            active_winding_pwm=150,
        )
        self.assertEqual(corrected, MusclePWM(150, -85, 0))
        self.assertEqual(
            allocator.elbow_cable_balance_status()[
                "triceps_release_extra_pwm"
            ],
            10.0,
        )

        allocator.virtual_cable_effort["biceps"] = 40.0
        corrected = allocator.apply_elbow_cable_balance(
            MusclePWM(-120, 150, 0),
            active_winding_pwm=150,
        )
        self.assertEqual(corrected, MusclePWM(-140, 150, 0))

    def test_elbow_cable_balance_never_releases_faster_than_active_winding(self):
        allocator = MuscleAllocator(
            cable_return_horizon=0.1,
            cable_return_max_pwm=60.0,
            enable_conditioning=False,
        )
        allocator.virtual_cable_effort["triceps"] = 500.0
        corrected = allocator.apply_elbow_cable_balance(
            MusclePWM(150, -75, 0),
            active_winding_pwm=150,
        )
        self.assertEqual(corrected, MusclePWM(150, -135, 0))
        self.assertEqual(
            allocator.elbow_cable_balance_status()[
                "triceps_release_extra_pwm"
            ],
        ...
        # A winding command must never be modified by the payout correction.
        allocator.virtual_cable_effort["biceps"] = 500.0
        winding = MusclePWM(80, -40, 0)
        self.assertEqual(
            allocator.apply_elbow_cable_balance(winding, 80).biceps,
            80,
        )
    ...
    def test_minimum_pwm_does_not_hold_motor_on_during_stop_or_reverse(self):
        allocator = MuscleAllocator(
            motor_min_pwm=(200, 200, 200),
            command_filter_tau=0.02,
            wind_slew_rate=10000.0,
            release_slew_rate=10000.0,
            enable_conditioning=True,
        )
        active = allocator.allocate("elbow", 200, now=0.00)
        self.assertEqual(active.biceps, 200)
        stopping = allocator.allocate("elbow", 0, now=0.02)
        self.assertGreater(stopping.biceps, 0)
        self.assertLess(stopping.biceps, 200)
        reversing = allocator.allocate("elbow", -200, now=0.04)
        self.assertGreaterEqual(reversing.biceps, 0)
        self.assertLess(reversing.biceps, 200)
    ...
    def test_shoulder_coupling_releases_then_rewinds_equal_pwm_time(self):
        allocator = MuscleAllocator(
            shoulder_elbow_coupling_pwm=50,
            enable_conditioning=False,
        )
        base = MusclePWM(0, 0, 30)
        released = allocator.apply_shoulder_elbow_coupling(
            base, shoulder_pwm=30, now=0.0
        )
        self.assertEqual(released, MusclePWM(-50, -50, 30))
        allocator.apply_shoulder_elbow_coupling(base, shoulder_pwm=30, now=0.1)
        self.assertAlmostEqual(
            allocator.shoulder_coupling_status()["released_effort_pwm_seconds"],
            5.0,
        )
        rewound = allocator.apply_shoulder_elbow_coupling(
            MusclePWM(0, 0, -30), shoulder_pwm=-30, now=0.2
        )
        self.assertEqual(rewound, MusclePWM(50, 50, -30))
        self.assertAlmostEqual(
            allocator.shoulder_coupling_status()["released_effort_pwm_seconds"],
            0.0,
        )

    def test_shoulder_coupling_rewind_ratio_changes_real_command(self):
        allocator = MuscleAllocator(
            shoulder_elbow_coupling_pwm=50,
        ...
        base = MusclePWM(0, 0, 30)
        allocator.apply_shoulder_elbow_coupling(base, shoulder_pwm=30, now=0.0)
        allocator.apply_shoulder_elbow_coupling(base, shoulder_pwm=30, now=0.1)
        allocator.apply_shoulder_elbow_coupling(base, shoulder_pwm=30, now=0.2)
        rewound = allocator.apply_shoulder_elbow_coupling(
            MusclePWM(0, 0, -30), shoulder_pwm=-30, now=0.3
        )
        self.assertEqual(rewound, MusclePWM(75, 75, -30))
        self.assertEqual(allocator.shoulder_coupling_status()["rewind_ratio"], 1.5)

    def test_shoulder_coupling_respects_deltoid_lock_and_triceps_cap(self):
        allocator = MuscleAllocator(shoulder_elbow_coupling_pwm=50)
        locked = allocator.apply_shoulder_elbow_coupling(
            MusclePWM(10, 150, 0),
            shoulder_pwm=50,
            deltoid_enabled=False,
            now=0.0,
        )
        self.assertEqual(locked, MusclePWM(10, 150, 0))
        self.assertEqual(
            allocator.cap_triceps_wind(locked, 100),
            MusclePWM(10, 100, 0),
        )
        self.assertEqual(
            allocator.cap_triceps_wind(MusclePWM(0, -150, 0), 100).triceps,
            -150,
        )

    def test_elbow_soft_landing_fades_pair_and_latches_after_target(self):
        allocator = MuscleAllocator(
            elbow_soft_landing_target=90,
            elbow_soft_landing_zone=25,
            elbow_soft_landing_min_ratio=0.25,
        )
        pair = MusclePWM(100, -50, 0)
        self.assertEqual(
            allocator.apply_elbow_soft_landing(pair, 65), pair
        )
        halfway = allocator.apply_elbow_soft_landing(pair, 77.5)
        self.assertEqual(halfway, MusclePWM(62, -31, 0))
        at_target = allocator.apply_elbow_soft_landing(pair, 90)
        self.assertEqual(at_target, MusclePWM(25, -12, 0))
        crossed = allocator.apply_elbow_soft_landing(pair, 95)
        self.assertEqual(crossed, MusclePWM(25, -12, 0))

        # Reversing away from the protected center releases the latch.
        leaving = allocator.apply_elbow_soft_landing(
            MusclePWM(-80, 100, 0), 88
        )
        self.assertEqual(leaving, MusclePWM(-80, 100, 0))
    ...
    def test_following_output_uses_velocity_only_for_direction_and_trigger(self):
        system = IMURehabSystem(
        ...
        self.assertEqual(system.compute_following_output(40.0), 8.0)
        self.assertEqual(system.compute_following_output(-40.0), -8.0)

    def test_gui_following_pwm_ten_is_never_promoted_to_two_hundred(self):
        system = IMURehabSystem(
            controller_mode="following_only",
            assist_feedforward_min_pwm=10.0,
            assist_feedforward_gain=99.0,
            following_velocity_threshold=5.0,
        )
        self.assertEqual(system.compute_following_output(5.1), 10.0)
        self.assertEqual(system.compute_following_output(1000.0), 10.0)
        self.assertEqual(system.compute_following_output(-1000.0), -10.0)
        allocator = MuscleAllocator(
            motor_min_pwm=(200, 200, 200),
            enable_conditioning=False,
        ...
        self.assertEqual(
            allocator.allocate("elbow", 10.0, enforce_minimum=False),
            MusclePWM(10, -5, 0),
        )
    ...
    def test_following_only_uses_imu_motion_without_restarted_trajectory(self):
        ...
        self.assertEqual(system.compute_following_output(0.0), 0.0)
        self.assertEqual(system.compute_following_output(20.0), 10.0)
        self.assertEqual(system.compute_following_output(-20.0), -10.0)
        self.assertFalse(system.uses_restarted_trajectory())

    def test_sample_hold_following_samples_only_while_output_is_zero(self):
        system = IMURehabSystem(
            controller_mode="following_only",
            assist_feedforward_min_pwm=50.0,
            following_max_session_duration=0.50,
        )
        system.motor_enabled = True

        step = system.compute_sample_hold_following
        self.assertEqual(step(0.0, 0.0, 0.0, 0.0, 0.00), (0.0, 0.0))
        self.assertEqual(system.event_following_status("elbow")["state"], "blanking")
        self.assertEqual(step(0.0, 0.0, 0.0, 0.0, 0.04), (0.0, 0.0))
        self.assertEqual(system.event_following_status("elbow")["state"], "sensing")
        self.assertEqual(step(0.3, 100.0, -0.3, -100.0, 0.06), (0.0, 0.0))

        # Only displacement across the 40 ms motor-off window chooses the
        # held directions; instantaneous velocity arguments do not choose it.
        self.assertEqual(step(0.6, -100.0, -0.6, 100.0, 0.08), (50.0, -50.0))
        self.assertEqual(system.event_following_status("elbow")["state"], "drive")
        # Moderate live motion cannot change the held PWM during 200 ms.
        self.assertEqual(step(5.0, 50.0, -5.0, -50.0, 0.20), (50.0, -50.0))
        self.assertEqual(step(9.0, 50.0, -9.0, -50.0, 0.279), (50.0, -50.0))
        self.assertEqual(step(10.0, 50.0, -10.0, -50.0, 0.281), (0.0, 0.0))
        self.assertEqual(system.event_following_status("elbow")["state"], "blanking")

        # A second motor-off sample may create another held block, but the
        # complete movement session cannot exceed its independent time cap.
        self.assertEqual(step(10.0, 0.0, -10.0, 0.0, 0.321), (0.0, 0.0))
        self.assertEqual(step(10.3, 0.0, -10.3, 0.0, 0.341), (0.0, 0.0))
        self.assertEqual(step(10.6, 0.0, -10.6, 0.0, 0.361), (50.0, -50.0))
        self.assertEqual(step(12.0, 0.0, -12.0, 0.0, 0.55), (50.0, -50.0))
        self.assertEqual(step(12.6, 0.0, -12.6, 0.0, 0.581), (0.0, 0.0))
        self.assertEqual(system.event_following_status("elbow")["state"], "rearm")

    def test_sample_hold_following_is_disarmed_while_motor_disabled(self):
        system = IMURehabSystem(controller_mode="following_only")
        self.assertEqual(
            system.compute_sample_hold_following(0.0, 50.0, 0.0, 50.0, 0.0),
            (0.0, 0.0),
        )
        self.assertEqual(system.event_following_status("shoulder")["state"], "idle")
        system.motor_enabled = True
        system.compute_sample_hold_following(0.0, 12.0, 0.0, 12.0, 0.1)
        self.assertEqual(system.event_following_status("shoulder")["state"], "blanking")
        system.motor_enabled = False
        self.assertEqual(
            system.compute_sample_hold_following(2.0, 30.0, 2.0, 30.0, 0.2),
            (0.0, 0.0),
        )
        self.assertEqual(system.event_following_status("shoulder")["state"], "idle")

    def test_sample_hold_rejects_inconsistent_direction_votes(self):
        system = IMURehabSystem(
            controller_mode="following_only",
            assist_feedforward_min_pwm=50.0,
        )
        ...
        self.assertEqual(step(0.0, 0.0, 0.0, 0.0, 0.00), (0.0, 0.0))
        self.assertEqual(step(0.0, 0.0, 0.0, 0.0, 0.04), (0.0, 0.0))
        self.assertEqual(step(0.3, 0.0, 0.0, 0.0, 0.06), (0.0, 0.0))
        self.assertEqual(step(-0.6, 0.0, 0.0, 0.0, 0.08), (0.0, 0.0))
        self.assertEqual(system.event_following_status("elbow")["state"], "sensing")

    def test_sample_hold_tracks_only_selected_joint(self):
        system = IMURehabSystem(
            controller_mode="following_only",
            assist_feedforward_min_pwm=50.0,
        )
        ...
        step = system.compute_sample_hold_following
        self.assertEqual(
            step(0.0, 0.0, 0.0, 0.0, 0.00, active_joint="elbow"),
            (0.0, 0.0),
        )
        self.assertEqual(
            step(0.0, 0.0, 0.0, 0.0, 0.04, active_joint="elbow"),
            (0.0, 0.0),
        )
        # Large shoulder-only motion must not start an elbow following pulse.
        self.assertEqual(
            step(0.0, 0.0, 0.3, 30.0, 0.06, active_joint="elbow"),
            (0.0, 0.0),
        )
        self.assertEqual(
            step(0.0, 0.0, 0.6, 30.0, 0.08, active_joint="elbow"),
            (0.0, 0.0),
        )
        self.assertEqual(system.event_following_status("elbow")["state"], "sensing")

        # After the rejected sample resets its baseline, elbow motion starts
        # only the elbow output; shoulder output remains zero.
        self.assertEqual(
            step(0.3, 30.0, 0.6, 0.0, 0.10, active_joint="elbow"),
            (0.0, 0.0),
        )
        self.assertEqual(
            step(0.6, 30.0, 0.6, 0.0, 0.12, active_joint="elbow"),
            (50.0, 0.0),
        )
    ...
    def test_compensation_limit_is_user_adjustable_up_to_physical_pwm_limit(self):
        system = IMURehabSystem(controller_mode="pid", output_limit=999.0)
        self.assertEqual(system.output_limit, 255.0)
        system.configure_control(output_limit=180.0)
        self.assertEqual(system.output_limit, 180.0)
        system.configure_control(output_limit=999.0)
        self.assertEqual(system.output_limit, 255.0)

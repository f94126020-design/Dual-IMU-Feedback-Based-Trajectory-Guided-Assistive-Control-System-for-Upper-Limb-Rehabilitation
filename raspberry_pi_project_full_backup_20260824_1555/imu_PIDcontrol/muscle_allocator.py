...
class MuscleAllocator:
    def __init__(
        ...
        self.shoulder_elbow_coupling_pwm = min(
            max(float(shoulder_elbow_coupling_pwm), 0.0), self.pwm_limit
        )
        self.shoulder_coupling_rewind_ratio = self.clamp(
            float(shoulder_coupling_rewind_ratio), 0.1, 2.0
        )
        self.shoulder_coupling_effort_limit = max(
            float(shoulder_coupling_effort_limit), 0.0
        )
        self.elbow_soft_landing_target = float(elbow_soft_landing_target)
        self.elbow_soft_landing_zone = max(float(elbow_soft_landing_zone), 1.0)
        self.elbow_soft_landing_min_ratio = self.clamp(
            float(elbow_soft_landing_min_ratio), 0.0, 1.0
        )
    ...
    def reset_conditioner(self):
        ...
        self.elbow_soft_landing_direction = 0
        self.elbow_soft_landing_last_scale = 1.0
    ...
    def reset_cable_tracker(self):
        ...
        self.shoulder_coupling_effort = 0.0
        self.shoulder_coupling_last_time = None
        self.shoulder_coupling_command = 0.0
        self.shoulder_coupling_phase = "idle"
        self.last_elbow_balance_correction = {
    ...
    def shoulder_coupling_status(self):
        """Return the encoder-free common-mode shoulder cable estimate."""
        return {
            "phase": self.shoulder_coupling_phase,
            "command_pwm": self.shoulder_coupling_command,
            "released_effort_pwm_seconds": self.shoulder_coupling_effort,
            "coupling_pwm": self.shoulder_elbow_coupling_pwm,
            "rewind_ratio": self.shoulder_coupling_rewind_ratio,
        }

    def elbow_cable_balance_status(self):
        return dict(self.last_elbow_balance_correction)

    def apply_elbow_cable_balance(self, pwm, active_winding_pwm):
        """Gradually repay estimated cable shortening on the next payout phase.

        The fixed 0.5/0.8 ratios remain the nominal geometry mapping. Actual
        short pulse/ramp asymmetry appears as positive virtual cable effort;
        this adds a bounded integral correction over ``cable_return_horizon``.
        """
        active = abs(float(active_winding_pwm))
        biceps = float(pwm.biceps)
        triceps = float(pwm.triceps)
        biceps_extra = 0.0
        triceps_extra = 0.0

        if biceps < 0.0 and active > 0.0:
            room = max(0.0, active - abs(biceps))
            biceps_extra = min(
                room,
                self.virtual_cable_effort["biceps"] / self.cable_return_horizon,
                self.cable_return_max_pwm,
            )
            biceps -= biceps_extra
        if triceps < 0.0 and active > 0.0:
            room = max(0.0, active - abs(triceps))
            triceps_extra = min(
                room,
                self.virtual_cable_effort["triceps"] / self.cable_return_horizon,
                self.cable_return_max_pwm,
            )
            triceps -= triceps_extra

        self.last_elbow_balance_correction = {
            "biceps_release_extra_pwm": float(biceps_extra),
            "triceps_release_extra_pwm": float(triceps_extra),
        }
        return MusclePWM(
    ...
    def apply_shoulder_elbow_coupling(
        self,
        pwm,
        shoulder_pwm,
        motor_enabled=True,
        emergency_stop=False,
        deltoid_enabled=True,
        now=None,
    ):
        """Add equal common-mode biceps/triceps payout for shoulder motion.

        Positive shoulder PWM raises the arm and releases both elbow cables.
        Negative shoulder PWM rewinds only the PWM-time previously released by
        this coupling. This is an encoder-free estimate and deliberately stops
        rewinding when shoulder lowering stops.
        """
        timestamp = time.monotonic() if now is None else float(now)
        if self.shoulder_coupling_last_time is None:
            dt = 0.0
        else:
            dt = max(0.0, min(timestamp - self.shoulder_coupling_last_time, 0.1))
        self.shoulder_coupling_last_time = timestamp

        common = 0.0
        phase = "idle"
        shoulder = float(shoulder_pwm)
        if motor_enabled and not emergency_stop and deltoid_enabled:
            if shoulder > 0.5:
                common = -self.shoulder_elbow_coupling_pwm
                phase = "release"
                self.shoulder_coupling_effort = self.clamp(
                    self.shoulder_coupling_effort + abs(common) * dt,
                    0.0,
                    self.shoulder_coupling_effort_limit,
                )
            elif shoulder < -0.5 and self.shoulder_coupling_effort > 0.0:
                available_pwm = (
                    self.shoulder_coupling_effort / dt
                    if dt > 1e-9 else self.shoulder_elbow_coupling_pwm
                )
                rewind_pwm = (
                    self.shoulder_elbow_coupling_pwm
                    * self.shoulder_coupling_rewind_ratio
                )
                common = min(rewind_pwm, available_pwm)
                phase = "rewind"
                self.shoulder_coupling_effort = max(
                    0.0,
                    self.shoulder_coupling_effort - common * dt,
                )

        self.shoulder_coupling_command = float(common)
        self.shoulder_coupling_phase = phase
        return MusclePWM(
            biceps=self.clamp_pwm(pwm.biceps + common),
            triceps=self.clamp_pwm(pwm.triceps + common),
            deltoid=self.clamp_pwm(pwm.deltoid),
        )

    def cap_triceps_wind(self, pwm, positive_limit):
        """Limit triceps winding only; payout remains unrestricted."""
        limit = max(0, min(int(round(positive_limit)), self.pwm_limit))
        return MusclePWM(
            biceps=self.clamp_pwm(pwm.biceps),
            triceps=min(self.clamp_pwm(pwm.triceps), limit),
            deltoid=self.clamp_pwm(pwm.deltoid),
        )

    def apply_elbow_soft_landing(self, pwm, elbow_angle):
        """Fade the paired elbow command while it approaches/crosses the target."""
        direction = 1 if pwm.biceps > 0 else -1 if pwm.triceps > 0 else 0
        if direction == 0:
            self.elbow_soft_landing_last_scale = 1.0
            return pwm

        angle = float(elbow_angle)
        target = self.elbow_soft_landing_target
        zone = self.elbow_soft_landing_zone
        distance_before_target = (
            target - angle if direction > 0 else angle - target
        )
        approaching = 0.0 <= distance_before_target <= zone

        if (
            self.elbow_soft_landing_direction
            and direction != self.elbow_soft_landing_direction
        ):
            self.elbow_soft_landing_direction = 0
        if approaching:
            self.elbow_soft_landing_direction = direction

        if self.elbow_soft_landing_direction == direction:
            progress = self.clamp(distance_before_target / zone, 0.0, 1.0)
            scale = self.elbow_soft_landing_min_ratio + (
                1.0 - self.elbow_soft_landing_min_ratio
            ) * progress
        else:
            scale = 1.0

        self.elbow_soft_landing_last_scale = float(scale)
        return MusclePWM(
            biceps=self.clamp_pwm(float(pwm.biceps) * scale),
            triceps=self.clamp_pwm(float(pwm.triceps) * scale),
            deltoid=self.clamp_pwm(pwm.deltoid),
        )
    ...
    def allocate(
        ...
        def shape_output(value, motor_index):
            # The minimum PWM is starting/active-motion assistance, not a
            # stopping floor.  When the requested command becomes zero or
            # reverses, let the conditioned output decay below the minimum;
            # otherwise 150, 80, 20... would all be promoted back to 200 and
            # the tendon would keep winding after the patient stopped.
            same_requested_direction = (
                abs(requested_command) > 0.5
                and self.sign(u) == self.sign(requested_command)
            )
            if enforce_minimum and same_requested_direction:

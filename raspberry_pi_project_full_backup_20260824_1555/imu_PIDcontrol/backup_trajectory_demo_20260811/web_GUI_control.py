...
function updatePage(data){
  ...
  if(data.motor_output){
    ...
    setText('dryRunBanner','持續柔和助力 LIVE：目標速度前饋＋誤差 PID；肘肌反向 1:'+s.antagonist_release_gain
      +'、低通 '+s.command_filter_tau+'s、換向停頓 '+Math.round(s.reverse_deadtime*1000)+'ms；按鍵確認及 ESP32 ARM 成功後才輸出。');

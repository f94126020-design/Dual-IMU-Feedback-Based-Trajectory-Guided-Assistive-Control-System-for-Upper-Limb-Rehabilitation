    ...
    def allocate(
        ...
        if target_joint == "elbow":
            if u > 0:
                ...
                pwm.biceps = shape_output(u, 0)
                pwm.triceps = shape_output(-abs(u), 1)
            elif u < 0:
                # elbow extension assistance: triceps winds, biceps releases
                pwm.biceps = shape_output(-abs(u), 0)
                pwm.triceps = shape_output(abs(u), 1)
            if enforce_minimum:
                pwm = self.apply_cable_return_compensation(pwm, abs(u))
            if u != 0:
                # Tendon-pair invariant: both elbow motors receive exactly the
                # same magnitude in opposite directions, including minimum PWM.
                pair_magnitude = max(abs(pwm.biceps), abs(pwm.triceps))
                if u > 0:
                    pwm.biceps, pwm.triceps = pair_magnitude, -pair_magnitude
                else:
                    pwm.biceps, pwm.triceps = -pair_magnitude, pair_magnitude

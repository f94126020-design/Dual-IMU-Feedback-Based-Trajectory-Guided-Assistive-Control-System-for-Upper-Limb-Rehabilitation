...
<div class="container">
  <div>
    ...
    <div class="card">
      ...
      <details>
        <summary>固定測試架：單顆馬達接線測試</summary>
        <div class="validation warning">必須先 Disable Motor、卸除人體負載並固定機構。測試輸出 200 PWM、0.5 秒，結束後自動 STOP；此輸出約為 78% duty，請保持急停可立即操作。</div>
        <div class="grid2">
          <div><label>馬達</label><select id="motorTestChannel"><option value="biceps">二頭肌 Motor 1</option><option value="triceps">三頭肌 Motor 2</option><option value="deltoid">三角肌 Motor 3</option></select></div>
          <div><label>方向</label><select id="motorTestDirection"><option value="1">正向＋（預期收線）</option><option value="-1">反向－（預期放線）</option></select></div>
        </div>
        <button class="warning" onclick="testSingleMotor()">輸出單顆馬達測試脈衝</button>
      </details>
    </div>
...
async function testSingleMotor(){
  const motor=document.getElementById('motorTestChannel').value;
  const direction=parseInt(document.getElementById('motorTestDirection').value,10);
  const labels={biceps:'二頭肌 Motor 1',triceps:'三頭肌 Motor 2',deltoid:'三角肌 Motor 3'};
  const directionText=direction>0?'正向＋（預期收線）':'反向－（預期放線）';
  const confirmed=window.confirm('即將測試 '+labels[motor]+' '+directionText+'。\n\n請確認：\n1. 已按 Disable Motor\n2. 完全沒有連接人體\n3. 機構與馬達已固定\n4. 急停可立即操作\n\n系統將輸出 200 PWM（約 78% duty）、0.5 秒後自動 STOP。確定開始嗎？');
  if(!confirmed){showStatus('已取消單顆馬達測試。');return;}
  const res=await postJSON('/api/motor_test/pulse',{motor,direction,confirmed_rig:true});
  showStatus(res.message||'單顆馬達測試沒有回傳訊息。',5000);
}
...
async function pollStatus(){
  ...
  setTimeout(pollStatus,document.hidden?1000:250);
}
pollStatus();
    ...
    while reader_running:
        try:
            ...
            desired_pwm = muscle_allocator.allocate(
                ...
                # Identification/demo modes have their own deliberately low
                # safety caps and must never be promoted to the 200 PWM
                # therapeutic motor-start threshold.
                enforce_minimum=not frequency_running and frame.get("controller_mode") != "trajectory_demo",
                feedback=fb,
                now=frame.get("time"),
            )

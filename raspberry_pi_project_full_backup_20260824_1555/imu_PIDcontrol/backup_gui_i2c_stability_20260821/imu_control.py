...
class IMURehabSystem:
    ...
    def shoulder_tracking_enabled_from_down(self):
        ...
        return str(self.controller_mode).lower() in (
            "pid", "pi", "assist", "continuous_assist", "continuous",
    ...
    def compute_assist_feedforward(self, target_velocity):
        mode = str(self.controller_mode).lower()
        if mode not in ("continuous_assist", "continuous", "feedforward_assist", "feedforward_adrc"):
            return 0.0
    ...
    def compute_demo_feedforward(self, target_velocity):
        ...
        limit = min(abs(float(self.output_limit)), self.demo_output_limit, 255.0)
    ...
    def read_frame(self):
        ...
        if controlled_joint == "shoulder" and self.is_trajectory_demo():
            ...
            controlled_joint == "shoulder"
            and self.shoulder_tracking_enabled_from_down()
        ):
            # A shoulder tracking controller must be able to start assistance
            # from the natural-down position. Actual downward motion still
            # activates shoulder_release_command and the payout path.
        ...
        if controller_mode in ("adrc", "feedforward_adrc", "ilc_adrc"):
            ...
            feedback_pid_output = adrc_result["output"]
        ...
        if self.is_ilc_mode() and str(self.target_mode).lower() == "sine":
            ...
            ilc_output = self.ilc.output_at_phase(phase)
            wait_remaining = max(
                0.0,
            ...
            if self.motor_enabled and not self.emergency_stop and wait_remaining <= 0.0:
                ...
                valid_cycle = (
                    abs(base_output + ilc_output) < self.output_limit - 1e-6
        ...
        pid_output = self.clamp(
            feedback_pid_output + feedforward_output + ilc_output,
            -self.output_limit,
            self.output_limit,
        )

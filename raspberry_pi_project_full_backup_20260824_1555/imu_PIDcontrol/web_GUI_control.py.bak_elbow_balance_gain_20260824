...
FOLLOWING_TRICEPS_WIND_LIMIT = 100
  ...
  <div class="controls">
    ...
    <div class="card controller-card collapsible-card collapsed">
      ...
      <div id="feedforwardParameterPanel" class="parameter-panel controller-panel">
        ...
        <p class="small">純跟隨使用折衷同步狀態機：馬達歸零隔離 40 ms、取樣角度 40 ms，再依取樣方向固定輸出此 PWM 200 ms。輸出期間的 IMU 不會改變或延長命令，連續階段最多 3 秒。</p>
      </div>
...
function updatePage(data){
  ...
  if(updateSlowUi && data.motor_output){
    ...
    const coupling=s.shoulder_elbow_coupling||{};
    const elbowBalance=s.elbow_cable_balance||{};
    const couplingSummary='\n肩肘繩索連動：肩抬舉時二頭／三頭同時放線 '+(coupling.coupling_pwm||50)+' PWM；下放時依已放出的 PWM·時間等量收回。純跟隨三頭收線上限 '+s.following_triceps_wind_limit+' PWM。';
    const elbowBalanceSummary='\n肘部繩長平衡：二頭額外放線 '+Number(elbowBalance.biceps_release_extra_pwm||0).toFixed(1)+' PWM，三頭額外放線 '+Number(elbowBalance.triceps_release_extra_pwm||0).toFixed(1)+' PWM。';
    setText('dryRunBanner',(demo
      ?'開迴路展示 LIVE：只依目標速度輸出，IMU 僅記錄；上限 '+s.demo_output_limit+' PWM、啟動停留 1 秒。僅限空載／固定測試架，禁止連接人體。'
      :followingOnly
      ?'折衷取樣跟隨 LIVE：只跟隨'+((data.frame||{}).target_joint==='shoulder'?'肩關節':'肘關節')+'；馬達歸零 '+(((s.event_following||{}).blanking_duration_seconds||.04)*1000)+' ms＋取樣 '+(((s.event_following||{}).sample_duration_seconds||.04)*1000)+' ms＋固定 '+s.feedforward_min_pwm+' PWM 輸出 '+(((s.event_following||{}).hold_duration_seconds||.20)*1000)+' ms；連續階段最多 '+((s.event_following||{}).maximum_session_seconds||3)+' 秒。'
      :'IMU 動作跟隨 LIVE：實際角速度收放線＋大幅落後補償；三頭放線比 '+s.triceps_release_ratio+'、二頭放線比 '+s.biceps_release_ratio+'、低通 '+s.command_filter_tau+'s、換向停頓 '+Math.round(s.reverse_deadtime*1000)+'ms；按鍵確認及 ESP32 ARM 成功後才輸出。')+couplingSummary+elbowBalanceSummary);
...
def serialize_frame(frame):
    ...
    keys_float = [
        ...
        "virtual_cable_effort_biceps","virtual_cable_effort_triceps","virtual_cable_effort_deltoid","shoulder_coupling_pwm","shoulder_coupling_effort","biceps_balance_release_extra_pwm","triceps_balance_release_extra_pwm",

...
class IMURehabSystem:
    ...
    def configure_control(self, reset_controller=True, **kwargs):
        ...
        allowed = {
            ...
            "controller_mode", "deadband", "assist_delay", "output_limit",
    ...
    def get_raw_data(self):
        ...
        ay = self.read_word_2c(self.ACCEL_YOUT_H) / 16384.0
        az = self.read_word_2c(self.ACCEL_ZOUT_H) / 16384.0

        gx = self.read_word_2c(self.GYRO_XOUT_H) / 131.0
        gy = self.read_word_2c(self.GYRO_YOUT_H) / 131.0
        gz = self.read_word_2c(self.GYRO_ZOUT_H) / 131.0
    ...
    def read_frame(self):
        ...
        if self.elbow_axis == "pitch":
            ...
            signed_elbow_angle = forearm_pitch - upper_pitch
            raw_omega = self.omega_sign * (
                gyro_corr_data[1]["gy"] - gyro_corr_data[0]["gy"]
            )
        else:
            upper_angle = upper_roll
            forearm_angle = forearm_roll
            signed_elbow_angle = forearm_roll - upper_roll
            raw_omega = self.omega_sign * (
                gyro_corr_data[1]["gx"] - gyro_corr_data[0]["gx"]
            )

        elbow_angle_raw = abs(signed_elbow_angle)
        ...
        if abs(dx) >= abs(dz):
            shoulder_plane = "X軸主導抬舉"
        else:
            shoulder_plane = "Z軸主導抬舉"
        ...
        if self.shoulder_angle_velocity > self.shoulder_vel_threshold:
        ...
        if mode == "p":
            ...
            use_i = True
            use_d = True
        elif mode in ("assist", "assist_as_needed", "aan"):
            # 按需輔助：短暫誤差先用 P；持續跟不上才讓 I 項介入；D 項提供阻尼。
            use_p = abs(self.error_for_control) > 0.0
            use_i = self.error_active_time >= self.assist_delay
            use_d = abs(self.error_for_control) > 0.0
        else:
            # off 或未知模式：不輸出控制。
            use_p = False
            use_i = False
        ...
        if use_i and abs(self.error_for_control) > 0.0:
            self.integral_error += self.error_for_control * dt
            self.integral_error = self.clamp(
                self.integral_error,
                -self.integral_limit,
                self.integral_limit,
            )
        else:
            # 沒在需要輔助時，讓積分逐漸回到 0，避免病人已跟上但 Ki 還殘留。
